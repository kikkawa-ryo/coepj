...
, get_columns as (
    select
        ...
        case
            ...
            when commissions.{{ column }}.text is not null then json_extract_scalar(commissions.{{ column }}, "$.text")
            when commissions.{{ column }} is not null then json_extract_scalar(commissions, "{{'$.'~column}}")
            {%- endfor %}
            else null
