...
with
    ...
    pre_processing_0 as (
        ...
        union all
        select
            id, "other" as attributes, normalize(other_detail, nfkc) as description_str,
        from raw_table
        union all
        select
            id,
    ...
    {% for fix_dict in fix_dict_list %}
        ...
        {%- if loop.last %}
            , pre_processing as (select * from {{ "pre_processing_" ~ current_index }})
        {% endif -%}
    {% endfor -%},
    ...
    str2arr as (
        select id, attributes, split(description_str, ",") as description_arr,
        from process_for_array
    ),
    flatten as (
        select id, attributes, description_str
                ...
                then
                    rtrim(
                        repeat(
                            ...
                            cast(regexp_extract(description_str, r"\d+") as int64)
                        ),
                        ","
                    )
    ...
    processing_0 as (
        select
            id, attributes, trim(trim(lower(description_str), ",")) as description_str
